...
<thead>
 <tr>
  ...
  <th style="text-align: right;">RI4 Price&nbsp;/&nbsp;NSI-MW Sell<br>Quantity</th>
...
<?php
    ...
    if ($market)
        foreach ($market as &$product)
        {
            ...
            $nsimw_sell = $product['ps'];
            $nsimw_sell_volume = $product['psv'];
            ...
            if (is_null($ri4_price)) {
                if (is_null($nsimw_sell_volume) || !$nsimw_sell_volume)
            ...
            if (!is_null($ri4_price)) {
                ...
                if (!is_null($nsimw_sell) && ($nsimw_sell < $ri4_price) && ($nsimw_sell_volume > $ri4_market_quantity)) {
                    $interrupt_detected = true;
                    $warnings .= '<span class="label label-interrupt">interrupt</span>&nbsp;';
                }
            }
            if ($nsimw_sell_volume > $ri4_market_quantity) {
                // рассчитываем минимальную цену, ниже которой закупку производить не следует - позиция перебита конкурентами
                $min_buy_price = $nsimw_sell / (1.0+TAX_AND_FEE+MIN_PROFIT);
...
<?php
    ...
    if (is_null($nsimw_sell) && is_null($ri4_price))
    {
        ?><td></td><?php
    }
    else
    {
        ...
        if (!is_null($nsimw_sell))
        {
            if (!is_null($ri4_price) && ($ri4_price == $nsimw_sell) && ($ri4_market_quantity == $nsimw_sell_volume))
            {
            }
            else
            {
                if ($ri4_price == $nsimw_sell) { ?><span class="text-muted-much"><?php }
                ?><br><?=number_format($nsimw_sell,2,'.',',')?>&nbsp;<mark><?=number_format($nsimw_sell_volume,0,'.',',')?></mark><?php
                if ($ri4_price == $nsimw_sell) { ?></span><?php }
            }
        }
...
<thead>
 <tr>
  ...
  <th style="text-align: right;">NSI-MW Sell<br><mark>Quantity</mark></th>
  <th style="text-align: right;">Jita Buy..Sell<br><mark>Import Price</mark></th>
  <th style="text-align: right;">Amarr<br>Sell</th>
  <th style="text-align: right;">Universe<br>Price</th>
 </tr>
...
<tr>
 ...
 <?php } ?>
 <?php if (is_null($nsimw_sell)) { ?><td></td><?php } else { ?>
 <td align="right"><?=number_format($nsimw_sell,2,'.',',')?><br><mark><?=number_format($nsimw_sell_volume,0,'.',',')?></mark></td>
 <?php } ?>
...
<?php
    ...
    __dump_header("Malpais Market", FS_RESOURCES);
...
select
  ...
  nsimw_hub.ethp_sell as ps, -- NSI-MW sell
  nsimw_hub.ethp_sell_volume as psv -- NSI-MW sell volume
from
  ...
  ( select distinct type_id
    ...
    left outer join (
      ...
      from qi.esi_trade_hub_prices
      where ethp_location_id=1022822609240  -- станка рынка
    ) nsimw_hub on (market.type_id = nsimw_hub.ethp_type_id)
where
...
from
  ...
  qi.esi_corporation_assets hangar
    ...
    left outer join (
      ...
      from qi.esi_trade_hub_prices
      where ethp_location_id=1022822609240  -- станка рынка
    ) nsimw_hub on (hangar.eca_type_id = nsimw_hub.ethp_type_id)
