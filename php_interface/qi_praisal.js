...
function fillMarketHubsTable(tbody,active) {
 ...
 for (const h of g_market_hubs) {
  ...
  var margin = h[4];
  rows=rows+'<tr>'+
   '<td style="white-space: nowrap;">'+h[11]+'<br><grayed>'+h[10]+' ('+h[0]+')</grayed></td>';
  ...
  if (h[12]===null) rows=rows+'<td></td>'; else rows=rows+'<td>'+numLikeEve(h[12])+'</td>';
  if (h[13]===null) rows=rows+'<td></td>'; else rows=rows+'<td>'+numLikeEve(h[13])+'</td>';
...
$("#frmCorpAssets").on("submit", function(e){
 ...
 $.ajax({
  ...
  success: function(data){
   ...
   $(data).each(function(i,row) {
    var tr="<tr>"+
     "<td>"+((row.nm===null)?'':row.nm)+"</td>"+
     "<td>"+((row.lnm===null)?'':(row.lnm+'<br>'))+"<grayed>"+row.lfl+" ("+row.lid+")</grayed>"+"</td>"+
