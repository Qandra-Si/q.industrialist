...
<thead>
 <tr>
  ...
  <th style="text-align: center;">Profit,&nbsp;ISK/m³<br><small>Jita&nbsp;Sell&nbsp;-7.5%</small></th>
  <th style="text-align: right;">Packaged,&nbsp;m³</th>
  <th style="text-align: right;" class="text-info"><?=$trade_hub_system?> Buy<br><br>Volume</th>
...
?>
 ...
 <td align="right" class="text-info"><?=number_format($irmalin_buy,2,'.',',')?><br><br><?=number_format($irmalin_volume,0,'.',',')?></td>
...
<?php
    ...
    else
    {
        ...
        if ($irmalin_buy < $querious_non_profitable)
        {
...
select 
 ...
 irmalin.ethp_buy as ib, -- irmalin buy
 round(jita.ethp_buy::numeric,2) as jb, -- jita buy
 round(jita.ethp_sell::numeric,2) as js, -- jita sell
 irmalin.ethp_buy_volume as iv, -- irmalin volume
 jita.ethp_sell_volume as jv, -- jita volume
 round((((irmalin.ethp_buy*0.925) - jita.ethp_sell) / tid.sdet_packaged_volume)::numeric,2) as p, -- profit/куб.м (-7.5%)
...
where
 ...
 irmalin.ethp_buy > jita.ethp_sell and
 irmalin.ethp_buy_volume > 0 and
 jita.ethp_sell_volume > 0
order by 9 desc;
EOD;
    $params = array($trade_hub_id);
